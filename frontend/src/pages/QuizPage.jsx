...
import { useParams, useNavigate }Grom 'react-router-dom';
...
const QuizPage = () => {
  ...
  // Kullanıcının verdiği cevapları ve soru ID'lerini tutmak için ref
  const answeredQuestionsDetailsRef = useRef([]);
  const quizStartTimeRef = useRef(null); // Quiz başlangıç zamanı
  ...
  useEffect(() => {
    loadQuestions();
    quizStartTimeRef.current = Date.now(); // Quiz başladığında zamanı kaydet
    answeredQuestionsDetailsRef.current = []; // Her yeni quizde sıfırla
  }, [courseType]);
  ...
  const loadQuestions = async () => {
    try {
      setLoading(true);
      setError('');
      const questionLimit = 10; // Bu değer backend'den veya ayar olarak da gelebilir
      let response;

      // ParsedCourseInfo'yu kullanarak quiz bilgilerini ayarla
      setQuizInfo({
          difficulty: parsedCourseInfo.difficulty,
          displayName: difficultyUtils.getDisplayName(parsedCourseInfo.difficulty),
          icon: difficultyUtils.getIcon(parsedCourseInfo.difficulty)
      ...
      if (parsedCourseInfo.isGeneral || parsedCourseInfo.difficulty === 'mixed') {
        ...
        // TODO: 'weakness-training' gibi özel kurs türleri için de buraya mantık eklenecek
  ...
  const handleSubmitAnswer = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        // Cevaplanan soru detaylarını kaydet
  ...
  const handleNextQuestion = async () => { // async yapıldı
    if (currentQuestionIndex < questions.length - 1) {
      ...
      // Quiz bitti, sonuçları kaydet
      setSubmitting(true); // Kaydetme işlemi için submitting state'ini kullanabiliriz
      try {
        const quizEndTime = Date.now();
        const durationInSeconds = Math.round((quizEndTime - quizStartTimeRef.current) / 1000);

        const sessionDetails = {
            course_type: courseType, // Orijinal courseType (örn: 'general', 'difficulty-beginner')
        ...
        // Kayıt başarılı olduktan sonra anasayfaya yönlendir
        const finalScoreForState = { // Navigate state'i için hazırlık
        ...
        navigate('/', { state: { quizCompleted: true, score: finalScoreForState } });
          ...
          // Opsiyonel: Kullanıcıya bir "Tekrar Dene" butonu gösterilebilir veya anasayfaya yine de yönlendirilebilir
          // Şimdilik hata mesajı gösterip, kullanıcıyı QuizPage'de tutuyoruz.
          // navigate('/', { state: { quizCompleted: true, score: score, errorSaving: true } }); // Hata ile yönlendirme
      } finally {
          setSubmitting(false);
      }
    }
  ...
  if (error && !questions.length) return ( // Hata varsa ve soru yoksa göster
  ...
  if (!questions.length && !loading) return ( // Soru yoksa ve yükleme bitmişse göster
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="text-center max-w-md">
  ...
  if (!currentQuestion && questions.length > 0) { // Bu durum nadir olmalı ama bir guard
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {/* Hata mesajı quiz sırasında oluşursa (örn: submit ederken) */}
