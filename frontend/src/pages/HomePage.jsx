...
const HomePage = () => {
  ...
  // YENİ: handleStartCourse fonksiyonu güncellendi
  const handleStartCourse = (courseId) => { //
    setShowQuizResults(false); //
    if (courseId === 'learning-history') { // 'previous' ID'sini 'learning-history' olarak değiştirdik
      navigate('/learning-history');
    } else if (courseId === 'weakness-training') { // 'incorrect' ID'sini 'weakness-training' olarak değiştireceğiz
      navigate('/quiz/weakness-training'); // Weakness training özel quiz yolu
    } else {
  ...
  // YENİ: "futureCourses" dizisi güncellendi
  const otherCourses = [ //
    {
      id: 'learning-history', // ID güncellendi
      title: 'Learning History', //
      description: 'Review your past questions and track your learning journey over time.', //
      icon: '📈', //
      buttonText: 'View History', // Metin güncellendi
      buttonColor: 'btn-info', // Renk isteğe bağlı olarak değiştirilebilir (btn-secondary yerine)
      isActive: true, // Aktif yapıldı
      difficulty: 'Review', //
    ...
    {
      id: 'weakness-training', // ID güncellendi (sonraki adımda kullanılacak)
      title: 'Weakness Training', //
      description: 'Focus on questions you struggled with to turn weaknesses into strengths.', //
      icon: '🎯', //
      buttonText: 'Coming Soon', // Şimdilik böyle kalsın
      buttonColor: 'btn-secondary', //
      isActive: false, // Şimdilik false kalsın
      difficulty: 'Targeted', //
  ...
  const allCourses = [...baseCourses, ...difficultyCourses, ...otherCourses].map(course => { //
    ...
    return {
        ...course,
        stats: stats ? { completed: stats.completed, accuracy: `${stats.accuracy}%` , progress: 0 } : { completed: 0, accuracy: '0%', progress: 0 } //
    };
  });

  // ... (geri kalan render kodları)
  if (loading) { //
    return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {allCourses.map((course) => ( //
            <CourseCard
              key={course.id}
              course={course}
              onStartCourse={handleStartCourse} // handleStartCourse olarak güncellendi
              className="h-full"
            />
          ))}
        </div>
